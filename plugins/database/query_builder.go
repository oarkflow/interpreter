...
// identifierPattern restricts SQL identifiers (table/column names) to plain
// or dotted (table.column) forms of letters, digits and underscores, so they
// can never carry SQL metacharacters when spliced into query text.
...
// validOrderByEntry validates an order-by entry, which may be a bare
// identifier or an identifier followed by an ASC/DESC direction keyword.
func validOrderByEntry(s string) bool {
	fields := strings.Fields(strings.TrimSpace(s))
	switch len(fields) {
	case 1:
		return validIdentifier(fields[0])
	case 2:
		dir := strings.ToUpper(fields[1])
		return validIdentifier(fields[0]) && (dir == "ASC" || dir == "DESC")
	default:
		return false
	}
}

// allowedOperators is the vocabulary of comparison operators permitted in
// where()-style clauses; it covers both the operators this package's own
// methods generate internally (e.g. sqlOperatorForToken) and the operators
// a script author may pass directly to where(col, op, val).
var allowedOperators = map[string]bool{
	"=": true, "!=": true, "<>": true, "<": true, "<=": true, ">": true, ">=": true,
	"like": true, "not like": true, "in": true, "not in": true, "is": true, "is not": true,
}

func validOperator(op string) bool {
	return allowedOperators[strings.ToLower(strings.TrimSpace(op))]
}
...
func GetQueryBuilderProperty(qb *QueryBuilder, name string) object.Object {
	...
	case "from":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			if !validIdentifier(s.Value) {
				return object.NewError("from() table name %q is not a valid identifier", s.Value)
			}
			qb.mu.Lock()
	...
	case "where":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			if !validIdentifier(col.Value) {
				return object.NewError("where() column %q is not a valid identifier", col.Value)
			}
			qb.mu.Lock()
			if len(args) == 2 {
				...
				if !validOperator(op.Value) {
					qb.mu.Unlock()
					return object.NewError("where() operator %q is not allowed", op.Value)
				}
				qb.whereConds = append(qb.whereConds, whereClause{
					column:   col.Value,
					operator: op.Value,
	...
	// where_raw accepts a raw SQL condition fragment and must never be passed unsanitized external/user input.
	case "where_raw":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			if len(args) < 1 {
	...
	case "where_in":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			if !validIdentifier(col.Value) {
				return object.NewError("where_in() column %q is not a valid identifier", col.Value)
			}
	...
	case "where_between":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			if !validIdentifier(col.Value) {
				return object.NewError("where_between() column %q is not a valid identifier", col.Value)
			}
			qb.mu.Lock()
			qb.whereConds = append(qb.whereConds, whereClause{
	...
	case "where_like":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			if !validIdentifier(col.Value) {
				return object.NewError("where_like() column %q is not a valid identifier", col.Value)
			}
	...
	case "where_null", "where_not_null":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			if !validIdentifier(col.Value) {
				return object.NewError("%s() column %q is not a valid identifier", name, col.Value)
			}
			op := "IS NULL"
		...
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			...
			for _, pair := range filter.Pairs {
				...
				if !validIdentifier(key) {
					return object.NewError("where_filter() key %q is not a valid identifier", key)
				}
		...
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			for _, arg := range args {
				if s, ok := arg.(*object.String); ok {
					if !validOrderByEntry(s.Value) {
						return object.NewError("order_by() entry %q is not a valid identifier or direction", s.Value)
					}
					qb.mu.Lock()
	...
	// join accepts a raw SQL join clause and must never be passed unsanitized external/user input.
	case "join":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			if len(args) < 1 {
	...
	case "group_by":
		return &object.Builtin{Fn: func(args ...object.Object) object.Object {
			for _, arg := range args {
				if s, ok := arg.(*object.String); ok {
					if !validIdentifier(s.Value) {
						return object.NewError("group_by() entry %q is not a valid identifier", s.Value)
					}
					qb.mu.Lock()
...
func buildWhereClausesFromPattern(pattern ast.Pattern, prefix string) ([]whereClause, error) {
	...
	case *ast.LiteralPattern:
		...
		if !validIdentifier(prefix) {
			return nil, fmt.Errorf("field %q is not a valid identifier", prefix)
		}
...
func builtinQuery(args ...object.Object) object.Object {
	...
	if len(args) >= 2 {
		if s, ok := args[1].(*object.String); ok {
			if !validIdentifier(s.Value) {
				return object.NewError("query() table name %q is not a valid identifier", s.Value)
			}
			qb.tableName = s.Value
		}
	}
...
func builtinLazyQuery(env *object.Environment, args ...object.Object) object.Object {
	...
	if !validIdentifier(table.Value) {
		return object.NewError("lazy_query() table name %q is not a valid identifier", table.Value)
	}
