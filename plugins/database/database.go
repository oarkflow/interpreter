...
// runtimeContextWithTimeout derives a bounded context from the runtime context
// when timeout > 0; otherwise it returns the unmodified runtime context (whole
// -script execution budget) with a no-op cancel func, mirroring
// builtins/integrations.runtimeContextWithTimeout.
...
// extractTimeoutMs pulls an optional trailing timeout_ms INTEGER argument off
// args, returning the remaining args, the parsed timeout (0 if absent), and
// any error object. It only treats the last argument as timeout_ms when the
// argument count exceeds maxArgsWithoutTimeout, so existing call arities are
// completely unaffected when no timeout_ms is passed.
func extractTimeoutMs(args []object.Object, maxArgsWithoutTimeout int) ([]object.Object, time.Duration, object.Object) {
	if len(args) <= maxArgsWithoutTimeout {
		return args, 0, nil
	}
	last := args[len(args)-1]
	if last.Type() != object.INTEGER_OBJ {
		return nil, 0, object.NewError("timeout_ms must be INTEGER, got %s", last.Type())
	}
	ms := last.(*object.Integer).Value
	if ms <= 0 {
		return nil, 0, object.NewError("timeout_ms must be > 0")
	}
	return args[:len(args)-1], time.Duration(ms) * time.Millisecond, nil
}

// isDeadlineExceeded reports whether ctx was cancelled specifically because
// the per-call timeout derived via runtimeContextWithTimeout expired.
...
// timeoutErr builds the distinguishable "query timed out" error used when a
// query/exec is cancelled specifically because the caller-supplied
// timeout_ms budget (timeout > 0) expired, as opposed to any other query
// failure (syntax error, connection error, whole-script cancellation, etc).
func timeoutErr(timeout time.Duration) object.Object {
	return object.NewError("query timed out after %dms", timeout.Milliseconds())
}

func queryRows(ctx context.Context, timeout time.Duration, target dbQueryable, query string, params object.Object) (*squealx.Rows, object.Object) {
	...
	if named != nil {
		...
		if err != nil {
			if timeout > 0 && isDeadlineExceeded(ctx) {
...
func execStatement(ctx context.Context, timeout time.Duration, target dbQueryable, query string, params object.Object) (sql.Result, object.Object) {
	positional, named, errObj := parseDBParams(params)
	if errObj != nil {
		return nil, errObj
	}
...
func builtinDBQuery(env *object.Environment, args ...object.Object) object.Object {
	if len(args) < 2 || len(args) > 5 {
		return dbTupleResult(nil, "db_query requires 2-5 arguments: db_or_tx, query [, params] [, format] [, timeout_ms]")
	}
	args, timeout, errObj := extractTimeoutMs(args, 4)
	if errObj != nil {
		return dbTupleResult(nil, errObj.Inspect())
	}
	...
	defer cancel()
	rows, queryErr := queryRows(ctx, timeout, target, query, params)
	if queryErr != nil {
...
func builtinDBExec(env *object.Environment, args ...object.Object) object.Object {
	if len(args) < 2 || len(args) > 4 {
		return dbTupleResult(nil, "db_exec requires 2-4 arguments: db_or_tx, query [, params] [, timeout_ms]")
	}
	args, timeout, errObj := extractTimeoutMs(args, 3)
	if errObj != nil {
		return dbTupleResult(nil, errObj.Inspect())
	}
	target, errObj := dbTarget(args[0])
	...
	ctx, cancel := runtimeContextWithTimeout(env, timeout)
	defer cancel()
	result, execErr := execStatement(ctx, timeout, target, query, params)
	if execErr != nil {
...
func builtinDBTables(env *object.Environment, args ...object.Object) object.Object {
	...
	rows, queryErr := queryRows(runtimeContext(env), 0, target, query, object.NULL)
	if queryErr != nil {
		return dbTupleResult(nil, queryErr.Inspect())
	}
	defer rows.Close()
