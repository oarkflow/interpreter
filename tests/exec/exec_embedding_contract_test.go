...
func TestExecFileWithOptionsModuleDirOverride(t *testing.T) {
	...
	path := filepath.Join(dir, "entry.spl")
	if err := os.WriteFile(path, []byte(`import "math.spl" as math; math.answer;`), 0o600); err != nil {
		t.Fatalf("write entry fixture: %v", err)
	}
	res, err := ExecFileWithOptions(path, nil, ExecOptions{ModuleDir: dir})
	if err != nil {
