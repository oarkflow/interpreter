...
func TestImportSelectiveAliasesAndDeclarationExports(t *testing.T) {
	moduleDir, err := os.MkdirTemp(".", "module-alias-test-")
	if err != nil {
		t.Fatal(err)
	}
	...
	modulePath := filepath.Join(moduleDir, "api.spl")
	moduleContent := `
export function add(a, b) { return a + b; }
export type Result = Ok(value) | Err(message);
...
`
	...
	script := fmt.Sprintf(`import {add as plus, Ok as Success} from %q; [plus(20, 22), Success(7)];`, filepath.ToSlash(relPath))
	result := evalWithParserCheck(t, script, object.NewEnvironment())
	arr, ok := result.(*Array)
	if !ok || len(arr.Elements) != 2 {
		t.Fatalf("expected two results, got %T (%v)", result, result)
	}
	if got := arr.Elements[0].(*Integer).Value; got != 42 {
		t.Fatalf("expected aliased function result 42, got %d", got)
	}
	if got := arr.Elements[1].(*object.ADTValue).VariantName; got != "Ok" {
		t.Fatalf("expected aliased Ok constructor, got %q", got)
	}
}
