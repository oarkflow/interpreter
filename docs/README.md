...
This repository includes one self-contained feature showcase at `examples/all_in_one.spl`.
...
```bash
go run ./cmd/interpreter --profile untrusted examples/all_in_one.spl
...
```bash
go run ./cmd/spltool check --json examples/all_in_one.spl
go run ./cmd/spltool fmt examples/all_in_one.spl
...
go run ./cmd/spltool symbols --json examples/all_in_one.spl
go run ./cmd/spltool complete --prefix pri examples/all_in_one.spl
go run ./cmd/spltool hover --line 1 --col 1 examples/all_in_one.spl
go run ./cmd/spltool docs examples/all_in_one.spl
go run ./cmd/spltool session run --json --checkpoint baseline examples/all_in_one.spl
go run ./cmd/spltool session debug examples/all_in_one.spl
...
```bash
go run ./cmd/spltool session run --json --checkpoint baseline examples/all_in_one.spl
go run ./cmd/spltool session debug --json examples/all_in_one.spl
```
...
```go
result, err := interpreter.ExecFile("examples/all_in_one.spl", nil)
```
...
## Feature Showcase File

- `examples/all_in_one.spl`

This script demonstrates the language and core builtins end-to-end without
depending on companion SPL fixtures.
