...
func TestStringEssentialBuiltins(t *testing.T) {
	...
	if testEval(`regex_replace("abc-123", "[^a-z]+", "_");`).(*String).Value != "abc_" {
		t.Fatalf("unexpected regex_replace result")
	}
	if testEval(`trim_prefix("prefix-value", "prefix-");`).(*String).Value != "value" {
		t.Fatalf("unexpected trim_prefix result")
	}
	if testEval(`trim_suffix("value.spl", ".spl");`).(*String).Value != "value" {
		t.Fatalf("unexpected trim_suffix result")
	}
	if testEval(`pad_left("7", 3, "0");`).(*String).Value != "007" {
		t.Fatalf("unexpected pad_left result")
	}
	if testEval(`pad_right("7", 3, "0");`).(*String).Value != "700" {
		t.Fatalf("unexpected pad_right result")
	}
	if testEval(`"apple".upper();`).(*String).Value != "APPLE" {
		t.Fatalf("unexpected string method upper result")
	}
	if testEval(`"apple".toUpperCase();`).(*String).Value != "APPLE" {
...
func TestTimeEssentialBuiltins(t *testing.T) {
	...
	if testEval(`format_time_tz(0, "YYYY-MM-DD HH:mm", "America/New_York");`).(*String).Value != "1969-12-31 19:00" {
...
func TestMethodAndParseBuiltins(t *testing.T) {
	...
	if testEval(`(0).to_iso();`).(*String).Value != "1970-01-01T00:00:00Z" {
		t.Fatalf("unexpected to_iso result")
	}
	if testEval(`(0).format("YYYY-MM-DD HH:mm:ss");`).(*String).Value != "1970-01-01 00:00:00" {
	...
	// Parse builtins
	if testEval(`type(parse_float("3.14"));`).(*String).Value != "FLOAT" {
	...
	if testEval(`parse_string(123);`).(*String).Value != "123" {
		t.Fatalf("parse_string failed")
	}
	testIntegerObject(t, testEval(`parse_type("12", "int");`), 12)
	if testEval(`type(parse_type("12.5", "float"));`).(*String).Value != "FLOAT" {
