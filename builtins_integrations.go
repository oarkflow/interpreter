...
func tupleOK(v Object) Object {
	return &Array{Elements: []Object{v, NULL}}
}

func tupleErr(msg string) Object {
	return &Array{Elements: []Object{NULL, &String{Value: msg}}}
}

func tupleBoolErr(ok bool, msg string) Object {
	if ok {
...
func hashGetString(h *Hash, key string, required bool) (string, Object) {
	v, ok := hashGet(h, key)
	if !ok {
		if required {
			return "", newError("missing required config key %q", key)
		}
		return "", nil
	}
	...
	if v.Type() != STRING_OBJ {
		return "", newError("config key %q must be STRING/SECRET, got %s", key, v.Type())
	}
	return v.(*String).Value, nil
}

func hashGetInt(h *Hash, key string, def int, required bool) (int, Object) {
	v, ok := hashGet(h, key)
	if !ok {
		if required {
			return 0, newError("missing required config key %q", key)
		}
		return def, nil
	}
	if v.Type() != INTEGER_OBJ {
		return 0, newError("config key %q must be INTEGER, got %s", key, v.Type())
	}
	return int(v.(*Integer).Value), nil
}

func hashGetStringSlice(h *Hash, key string) ([]string, Object) {
	v, ok := hashGet(h, key)
	if !ok {
		return nil, nil
	}
	if v.Type() == STRING_OBJ {
		return []string{v.(*String).Value}, nil
	}
	if v.Type() != ARRAY_OBJ {
		return nil, newError("config key %q must be STRING or ARRAY, got %s", key, v.Type())
	}
	arr := v.(*Array)
	out := make([]string, 0, len(arr.Elements))
	for i, el := range arr.Elements {
		if el.Type() != STRING_OBJ {
			return nil, newError("config key %q element at index %d must be STRING, got %s", key, i, el.Type())
		}
		out = append(out, el.(*String).Value)
	}
	return out, nil
}

func objectToStringMap(obj Object) (map[string]string, Object) {
	if obj == nil || obj == NULL {
		return map[string]string{}, nil
	}
	if obj.Type() != HASH_OBJ {
		return nil, newError("headers must be HASH, got %s", obj.Type())
	}
	h := obj.(*Hash)
	...
	for _, pair := range h.Pairs {
		...
		if !ok {
			return nil, newError("headers key must be STRING")
		}
...
func parseHTTPBodyAndHeaders(args []Object, bodyIndex int) ([]byte, map[string]string, time.Duration, Object) {
	body := []byte{}
	headers := map[string]string{}
	timeout := 30 * time.Second

	if len(args) > bodyIndex {
		if args[bodyIndex].Type() == STRING_OBJ {
			body = []byte(args[bodyIndex].(*String).Value)
		} else if args[bodyIndex] != NULL {
			enc, err := json.Marshal(objectToNative(args[bodyIndex]))
			if err != nil {
				return nil, nil, 0, newError("failed to encode request body as JSON: %s", err)
			}
			body = enc
	...
	if len(args) > bodyIndex+2 {
		if args[bodyIndex+2].Type() != INTEGER_OBJ {
			return nil, nil, 0, newError("timeout_ms must be INTEGER, got %s", args[bodyIndex+2].Type())
		}
		ms := args[bodyIndex+2].(*Integer).Value
		if ms <= 0 {
			return nil, nil, 0, newError("timeout_ms must be > 0")
		}
...
func parseHTTPHeadersAndTimeout(args []Object, headersIndex int) (map[string]string, time.Duration, Object) {
	headers := map[string]string{}
	timeout := 30 * time.Second
	...
	if len(args) > headersIndex+1 {
		if args[headersIndex+1].Type() != INTEGER_OBJ {
			return nil, 0, newError("timeout_ms must be INTEGER, got %s", args[headersIndex+1].Type())
		}
		ms := args[headersIndex+1].(*Integer).Value
		if ms <= 0 {
			return nil, 0, newError("timeout_ms must be > 0")
		}
		timeout = time.Duration(ms) * time.Millisecond
	}
...
func doHTTPRequest(env *Environment, method, target string, body []byte, headers map[string]string, timeout time.Duration) Object {
	if err := checkNetworkAllowed(target); err != nil {
	...
	headersMap := make(map[HashKey]HashPair)
	for k, vals := range resp.Header {
		key := &String{Value: k}
		headersMap[key.HashKey()] = HashPair{Key: key, Value: &String{Value: strings.Join(vals, ",")}}
	}
	...
	payload := map[string]interface{}{
		...
		"headers":     &Hash{Pairs: headersMap},
	}
	return tupleOK(toObject(payload))
}

func builtinHTTPRequest(env *Environment, args ...Object) Object {
...
func builtinHTTPGet(env *Environment, args ...Object) Object {
...
func builtinHTTPPost(env *Environment, args ...Object) Object {
...
func builtinWebhook(env *Environment, args ...Object) Object {
	if len(args) < 2 || len(args) > 4 {
...
func builtinSMTPSend(args ...Object) Object {
	if len(args) != 1 {
	...
	if err := checkNetworkAllowed(addr); err != nil {
...
func ftpConnect(cfg *Hash) (*ftp.ServerConn, Object) {
	host, errObj := hashGetString(cfg, "host", true)
	if errObj != nil {
		return nil, errObj
	}
	...
	if err := checkNetworkAllowed(fmt.Sprintf("%s:%d", host, port)); err != nil {
		return nil, newError("network policy denied ftp connection: %s", err)
	}

	c, err := ftp.Dial(fmt.Sprintf("%s:%d", host, port), ftp.DialWithTimeout(time.Duration(timeoutMs)*time.Millisecond))
	if err != nil {
		return nil, newError("ftp dial failed: %s", err)
	}
	if err := c.Login(username, password); err != nil {
		_ = c.Quit()
		return nil, newError("ftp login failed: %s", err)
	}
	return c, nil
}

func builtinFTPList(args ...Object) Object {
	if len(args) != 2 {
		return tupleErr("ftp_list requires 2 arguments: config, remote_dir")
	}
	if args[0].Type() != HASH_OBJ || args[1].Type() != STRING_OBJ {
		return tupleErr("ftp_list expects (HASH, STRING)")
	}
	c, errObj := ftpConnect(args[0].(*Hash))
	if errObj != nil {
		return tupleErr(errObj.Inspect())
	}
	defer c.Quit()

	entries, err := c.List(args[1].(*String).Value)
	if err != nil {
	...
	return tupleOK(&Array{Elements: out})
}

func builtinFTPGet(args ...Object) Object {
	if len(args) != 3 {
		return tupleErr("ftp_get requires 3 arguments: config, remote_path, local_path")
	}
	if args[0].Type() != HASH_OBJ || args[1].Type() != STRING_OBJ || args[2].Type() != STRING_OBJ {
		return tupleErr("ftp_get expects (HASH, STRING, STRING)")
	}
	safePath, err := sanitizePath(args[2].(*String).Value)
	if err != nil {
	...
	r, err := c.Retr(args[1].(*String).Value)
	if err != nil {
...
func builtinFTPPut(args ...Object) Object {
	if len(args) != 3 {
	...
	safePath, err := sanitizePath(args[1].(*String).Value)
	if err != nil {
		return tupleErr(err.Error())
	}
	...
	if err := c.Stor(args[2].(*String).Value, bytes.NewReader(content)); err != nil {
		return tupleErr(fmt.Sprintf("ftp store failed: %v", err))
	}
	return tupleBoolErr(true, "")
}

func sftpConnect(cfg *Hash) (*sftp.Client, *ssh.Client, Object) {
	host, errObj := hashGetString(cfg, "host", true)
	if errObj != nil {
	...
	if err := checkNetworkAllowed(fmt.Sprintf("%s:%d", host, port)); err != nil {
		return nil, nil, newError("network policy denied sftp connection: %s", err)
	}
	...
	if privateKey != "" {
		...
		if err != nil {
			return nil, nil, newError("invalid private_key: %s", err)
		}
		authMethods = append(authMethods, ssh.PublicKeys(signer))
	}
	if len(authMethods) == 0 {
		return nil, nil, newError("sftp config requires password or private_key")
	}
	...
	if err != nil {
		return nil, nil, newError("sftp dial failed: %s", err)
	}
	sftpClient, err := sftp.NewClient(client)
	if err != nil {
		client.Close()
		return nil, nil, newError("sftp client init failed: %s", err)
	}
	return sftpClient, client, nil
}

func builtinSFTPList(args ...Object) Object {
	if len(args) != 2 {
	...
	c, sshClient, errObj := sftpConnect(args[0].(*Hash))
	if errObj != nil {
		return tupleErr(errObj.Inspect())
	}
	defer c.Close()
	...
	entries, err := c.ReadDir(args[1].(*String).Value)
	if err != nil {
...
func builtinSFTPGet(args ...Object) Object {
	if len(args) != 3 {
	...
	r, err := c.Open(args[1].(*String).Value)
	if err != nil {
...
func builtinSFTPPut(args ...Object) Object {
	if len(args) != 3 {
	...
	w, err := c.Create(args[2].(*String).Value)
	if err != nil {
