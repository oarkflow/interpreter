...
func TestRunConformance(t *testing.T) {
	dir := t.TempDir()
	script := `test "arithmetic" { assert_eq(40 + 2, 42); }`
	if err := os.WriteFile(filepath.Join(dir, "core_test.spl"), []byte(script), 0o600); err != nil {
		t.Fatalf("write conformance fixture: %v", err)
	}
	var stdout, stderr bytes.Buffer
	code := Run([]string{"conformance", dir}, strings.NewReader(""), &stdout, &stderr)
	if code != 0 {
