...
func EvalForPlayground(script string, opts PlaygroundOptions) PlaygroundResult {
	...
	_, err := overrideFn(policy, func() (object.Object, error) {
		...
		// Checked here (rather than relying on interpreter.go's own
		// pre-parse check) because the playground evaluates scripts
		// directly via these function variables, never through
		// interpreter.ExecWithOptions/ExecFileWithOptions.
		if violations, scanErr := security.ScanForHardcodedSecrets(policy, script); scanErr != nil {
			res.Error = scanErr.Error()
			res.ErrorKind = "validation"
			return nil, nil
