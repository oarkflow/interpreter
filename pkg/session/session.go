...
// ErrorKind classifies execution errors reported via ExecutionMetrics.
// Its values intentionally match the string values of the root package's
// ExecErrorKind (interpreter.go) so hosts comparing across both APIs see
// consistent classifications; pkg/session cannot import the root package
// (which imports pkg/session), so the values are duplicated here rather than
...
type Session struct {
	...
	// runCancel is the cancel function for the context governing whatever
	// Execute call is currently in flight, if any. It is guarded by mu since
	// Cancel() is expected to be called from a different goroutine than the
	// one running Execute(). It is nil whenever no execution is in flight,
	// so Cancel() on an idle session is a safe no-op.
...
// Cancel cancels the currently-running (or next-to-start) Execute call on
// this session by invoking the cancel function of the context it derives its
// deadline/cancellation from. If no execution is currently in flight, Cancel
// is a safe no-op. Cancel does not stop any execution already completed, and
// it only affects a single in-flight (or immediately upcoming, in a narrow
// race window) Execute call — it is not a queue/registry of executions.
...
func (s *Session) Execute(req ExecutionRequest) ExecutionResult {
	...
	// Derive a per-execution cancellation context so Cancel() (called from
	// another goroutine) can stop this specific Execute call. If a Timeout
	// is configured, that also governs this context's deadline; otherwise
	// the context only ever ends via an explicit Cancel() call.
	var execCtx context.Context
	var execCancel context.CancelFunc
	if s.opts.Timeout > 0 {
		execCtx, execCancel = context.WithTimeout(context.Background(), s.opts.Timeout)
	} else {
		execCtx, execCancel = context.WithCancel(context.Background())
	}
	s.mu.Lock()
	s.runCancel = execCancel
	s.mu.Unlock()

	// Eval only consults env.RuntimeLimits.Ctx when RuntimeLimits is non-nil,
	// so ensure one exists even for sessions configured with no other
	// step/depth/heap limits, otherwise Cancel() would have no effect.
	rl := s.env.RuntimeLimits
	rlCreated := false
	if rl == nil {
		rl = &object.RuntimeLimits{HeapCheckEvery: 128}
		s.env.RuntimeLimits = rl
		rlCreated = true
	}
	prevRLCtx := rl.Ctx
	rl.Ctx = execCtx
	...
	result.Metrics = metricsFor(s.env, start, result)
	result.Metrics.ErrorKind = classifyExecutionErrorKind(result.Error, execCtx)
	s.finish(result, req)
	return result
}

// classifyExecutionErrorKind inspects an execution's error message and the
// context it ran under to distinguish caller-initiated cancellation
// (ExecErrorCancelled in the root package) from a deadline elapsing
// (ExecErrorTimeout), matching the same message markers pkg/eval's
// runtimeLimitsEnter produces (see eval.go's runtimeLimitsEnter and
// interpreter.go's classifyRuntimeErrorMessage, which this mirrors for the
// session package since it cannot import the root package).
func classifyExecutionErrorKind(errMsg string, execCtx context.Context) ErrorKind {
	if errMsg == "" {
		return ErrorKindNone
	}
	lower := strings.ToLower(errMsg)
	switch {
	case strings.Contains(lower, "execution cancelled"), strings.Contains(lower, "execution timeout exceeded"):
		if execCtx != nil {
			switch execCtx.Err() {
		...
		if strings.Contains(lower, "execution timeout exceeded") {
			return ErrorKindTimeout
		}
		return ErrorKindCancelled
	case strings.Contains(lower, "denied"), strings.Contains(lower, "not allowed"):
		return ErrorKindPolicyDenied
	case strings.Contains(lower, "limit exceeded"), strings.Contains(lower, "recursion depth exceeded"), strings.Contains(lower, "heap usage exceeded"):
		return ErrorKindResourceLimit
	default:
