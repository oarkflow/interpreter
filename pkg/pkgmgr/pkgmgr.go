...
// verifiedChecksumCache memoizes successful checksum verifications performed
// during import resolution, keyed by resolved path, so that a dependency
// that is imported many times (e.g. inside a loop) is only hashed once per
// process lifetime as long as its mtime does not change. Entries are
// invalidated when the file's modification time changes.
var verifiedChecksumCache sync.Map // resolvedPath -> verifiedChecksumEntry

type verifiedChecksumEntry struct {
	modTime  int64
	checksum string
}

// verifyDependencyChecksum checks the on-disk checksum of resolvedPath against
// expected, using verifiedChecksumCache to avoid re-hashing unchanged files on
// every import resolution. An empty expected checksum means "not recorded" and
// is treated as a no-op (no verification performed).
func verifyDependencyChecksum(alias, resolvedPath, expected string) error {
	...
	if cached, ok := verifiedChecksumCache.Load(resolvedPath); ok {
		entry := cached.(verifiedChecksumEntry)
		if entry.modTime == modTime {
			if !strings.EqualFold(entry.checksum, expected) {
				return fmt.Errorf("dependency %q checksum mismatch: expected %s, got %s (run spltool mod tidy if this is expected)", alias, expected, entry.checksum)
			}
			return nil
		}
	}
	got, err := DependencyChecksum(resolvedPath)
	if err != nil {
		return fmt.Errorf("dependency %q checksum failed: %w", alias, err)
	}
	verifiedChecksumCache.Store(resolvedPath, verifiedChecksumEntry{modTime: modTime, checksum: got})
	if !strings.EqualFold(got, expected) {
		return fmt.Errorf("dependency %q checksum mismatch: expected %s, got %s (run spltool mod tidy if this is expected)", alias, expected, got)
	}
	return nil
}
...
func ResolveManifestImport(importPath, moduleDir, sourcePath string) (string, bool, error) {
	...
	if lockPath != "" {
		...
		if dep, ok := lock.Dependencies[alias]; ok {
			if err := verifyDependencyChecksum(alias, dep.ResolvedPath, dep.Checksum); err != nil {
				return "", true, err
			}
