...
var knownStdModuleExports = map[string][]string{
	...
	"cryptoextra":   {"bcrypt_hash", "bcrypt_verify", "jwt_encode", "jwt_decode"},

	"yaml":        {"config_load", "config_parse", "yaml_encode", "yaml_decode"},
	"config/yaml": {"config_load", "config_parse", "yaml_encode", "yaml_decode"},
	"builtins":    {},
...
func (c *staticChecker) walkStatement(stmt ast.Statement) {
	...
	case *ast.LetStatement:
		...
		for _, n := range names {
			if n != nil {
				if _, placeholder := c.macroParams[n.Name]; !placeholder {
	...
	case *ast.SelectStatement:
		for _, sc := range s.Cases {
			c.walkExpression(sc.Channel)
			c.pushScope()
			if sc.Value != nil {
				c.declare(sc.Value.Name, "variable")
			}
			c.walkBlock(sc.Body)
			c.popScope()
		}
	...
	case *ast.MacroDefinition:
		if s.Name != nil {
			c.declare(s.Name.Name, "macro")
			c.macroDefs[s.Name.Name] = s
		}
		c.pushScope()
		previousParams := c.macroParams
		c.macroParams = make(map[string]struct{}, len(s.Parameters))
		for _, param := range s.Parameters {
			if param != nil {
				c.macroParams[param.Name] = struct{}{}
				c.declare(param.Name, "parameter")
			}
		}
		c.walkBlock(s.Body)
		c.macroParams = previousParams
		c.popScope()
	}
}
...
func (c *staticChecker) walkExpression(expr ast.Expression) {
	...
	case *ast.MacroCallExpression:
		if definition := c.macroDefs[e.Name.Name]; definition != nil {
			for _, statement := range definition.Body.Statements {
				let, ok := statement.(*ast.LetStatement)
				if !ok || let.Name == nil {
					continue
				}
				for i, parameter := range definition.Parameters {
					if parameter.Name == let.Name.Name && i < len(e.Arguments) {
						if identifier, ok := e.Arguments[i].(*ast.Identifier); ok {
							c.declare(identifier.Name, "macro binding")
						}
					}
				}
			}
...
func (c *staticChecker) checkImport(s *ast.ImportStatement) {
	...
	if len(s.Names) > 0 {
		for i, n := range s.Names {
			...
			local := n
			if i < len(s.Specifiers) && s.Specifiers[i].Local != nil {
				local = s.Specifiers[i].Local
			}
			c.declare(local.Name, "import")
			if len(exports) > 0 {
...
func exportedSymbolsFromDeclaration(path, src string, stmt ast.Statement) []Symbol {
	...
	case *ast.ClassStatement:
		...
		line, col := findNamePosition(src, s.Name.Name)
		return []Symbol{{Name: s.Name.Name, Kind: "class", Path: path, Line: line, Column: col, Detail: s.Name.Name}}
	case *ast.InterfaceStatement:
		if s.Name == nil {
			return nil
		}
		line, col := findNamePosition(src, s.Name.Name)
		return []Symbol{{Name: s.Name.Name, Kind: "interface", Path: path, Line: line, Column: col, Detail: s.Name.Name}}
	case *ast.TypeDeclarationStatement:
		symbols := make([]Symbol, 0, len(s.Variants)+1)
		if s.Name != nil {
			line, col := findNamePosition(src, s.Name.Name)
			symbols = append(symbols, Symbol{Name: s.Name.Name, Kind: "type", Path: path, Line: line, Column: col, Detail: s.String()})
		}
		for _, variant := range s.Variants {
			if variant == nil || variant.Name == nil {
				continue
			}
			line, col := findNamePosition(src, variant.Name.Name)
			symbols = append(symbols, Symbol{Name: variant.Name.Name, Kind: "constructor", Path: path, Line: line, Column: col, Detail: variant.Name.Name})
		}
		return symbols
	default:
		return nil
