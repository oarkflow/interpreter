...
func TestCheckSourceResolvesClassInterfaceAndADTExports(t *testing.T) {
	dir := t.TempDir()
	module := `
export type Result = Ok(value) | Err(message);
export interface Named { name(): string; }
export class User implements Named { name(): string { return "Ada"; } }
`
	if err := os.WriteFile(filepath.Join(dir, "library.spl"), []byte(module), 0o600); err != nil {
		t.Fatal(err)
	}
	src := `import {Result, Ok as Success, Named, User} from "library.spl";
let user = new User();
let result = Success(user.name());
`
	report := CheckSource(filepath.Join(dir, "main.spl"), src)
	for _, diag := range report.Diagnostics {
		if diag.Code == "missing-import" {
			t.Fatalf("expected all declared exports to resolve, got diagnostic: %#v", diag)
		}
	}
}
