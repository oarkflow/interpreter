...
// TypeRef is the structured representation of an SPL type annotation. Name is
// populated for named/generic types, Arguments contains generic parameters,
// Union contains alternatives, and ArrayDepth/Nullable preserve suffix forms.
type TypeRef struct {
	Name       string
	Arguments  []*TypeRef
	Union      []*TypeRef
...
// ParseTypeRef builds a structured type tree from the canonical parser text.
func ParseTypeRef(input string) *TypeRef {
	input = strings.TrimSpace(input)
	if input == "" {
		return nil
	}
	if parts := splitTypeTopLevel(input, '|'); len(parts) > 1 {
		union := &TypeRef{Union: make([]*TypeRef, 0, len(parts))}
		for _, part := range parts {
			union.Union = append(union.Union, ParseTypeRef(part))
		}
		return union
	}
	t := &TypeRef{}
	...
	if open := strings.IndexByte(input, '<'); open >= 0 && strings.HasSuffix(input, ">") {
		t.Name = strings.TrimSpace(input[:open])
		for _, arg := range splitTypeTopLevel(input[open+1:len(input)-1], ',') {
			t.Arguments = append(t.Arguments, ParseTypeRef(arg))
		}
	} else {
		t.Name = input
	}
...
func splitTypeTopLevel(input string, delimiter rune) []string {
	depth := 0
	start := 0
	parts := []string{}
	for i, r := range input {
...
func (ie *IfExpression) String() string {
	...
	if ie.Consequence != nil {
		consequence = ie.Consequence.String()
	}
	if ie.Alternative == nil {
		return fmt.Sprintf("if %s %s", condition, consequence)
	}
	return fmt.Sprintf("if %s %s else %s", condition, consequence, ie.Alternative.String())
}
...
func (ls *LetStatement) String() string {
	...
	if ls.TypeName != "" {
		return fmt.Sprintf("%s %s: %s = %s;", kind, ls.Name.String(), ls.TypeName, ls.Value.String())
	}
	return fmt.Sprintf("%s %s = %s;", kind, ls.Name.String(), ls.Value.String())
}
...
func (cf *ClassField) String() string {
	...
	if cf.Value != nil {
		out.WriteString(" = ")
		out.WriteString(cf.Value.String())
	}
	out.WriteString(";")
	return out.String()
}
...
func (cm *ClassMethod) String() string {
	...
	for i, p := range cm.Parameters {
		...
		if cm.HasRest && i == len(cm.Parameters)-1 {
			out.WriteString("...")
		}
		out.WriteString(p.String())
		if i < len(cm.ParamTypes) && cm.ParamTypes[i] != "" {
			out.WriteString(": ")
			out.WriteString(cm.ParamTypes[i])
		}
		if i < len(cm.Defaults) && cm.Defaults[i] != nil {
			out.WriteString(" = ")
			out.WriteString(cm.Defaults[i].String())
		}
	}
	out.WriteString(")")
	if cm.ReturnType != "" {
		out.WriteString(": ")
		out.WriteString(cm.ReturnType)
	}
	out.WriteString(" ")
	if cm.Body != nil {
	...
	if len(is.Names) > 0 {
		...
		if len(is.Specifiers) > 0 {
			for _, spec := range is.Specifiers {
				part := spec.Imported.String()
				if spec.Local != nil && spec.Local.Name != spec.Imported.Name {
					part += " as " + spec.Local.String()
				}
...
func (ys *YieldStatement) String() string {
	if ys.Value != nil {
		return fmt.Sprintf("yield %s;", ys.Value.String())
	}
	return "yield;"
}
...
// MacroCallExpression preserves unevaluated arguments and an optional trailing
// block so the evaluator can perform hygienic AST substitution.
...
func (mc *MacroCallExpression) expressionNode() {}
func (mc *MacroCallExpression) String() string {
	var out strings.Builder
...
func (md *MacroDefinition) statementNode() {}
func (md *MacroDefinition) String() string {
	var out strings.Builder
...
func (sl *StreamLiteral) expressionNode() {}
func (sl *StreamLiteral) String() string {
	var out strings.Builder
...
func (fa *ForAwaitStatement) statementNode() {}
func (fa *ForAwaitStatement) String() string {
	var out strings.Builder
	out.WriteString("for await (")
	if fa.KeyName != nil {
		out.WriteString(fa.KeyName.String())
		out.WriteString(", ")
	}
	out.WriteString(fa.ValueName.String())
	out.WriteString(" in ")
	out.WriteString(fa.Iterable.String())
	out.WriteString(") ")
	out.WriteString(fa.Body.String())
	return out.String()
}
...
func (ss *SelectStatement) String() string {
	...
	for _, c := range ss.Cases {
		out.WriteString("case ")
		out.WriteString(c.Channel.String())
		if c.Value != nil {
			out.WriteString(" <- ")
...
// ChannelSendExpression sends Value to Channel using `channel <- value`.
type ChannelSendExpression struct {
	Channel Expression
	Value   Expression
}

func (e *ChannelSendExpression) expressionNode() {}
func (e *ChannelSendExpression) String() string {
	return e.Channel.String() + " <- " + e.Value.String()
}

// ChannelReceiveExpression receives a value using postfix `channel <-`.
type ChannelReceiveExpression struct{ Channel Expression }

func (e *ChannelReceiveExpression) expressionNode() {}
func (e *ChannelReceiveExpression) String() string  { return e.Channel.String() + " <-" }
