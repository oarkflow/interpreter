...
func toIntObject(arg object.Object) object.Object {
	val, err := convert.ToInt64With(eval.ToNative(arg), convert.AllowFloatToIntTruncate())
	if err != nil {
		return object.NewError("cannot convert %s to int: %s", arg.Type(), err)
	}
	return &object.Integer{Value: val}
}

func toFloatObject(arg object.Object) object.Object {
	val, err := convert.ToFloat64(eval.ToNative(arg))
	if err != nil {
		return object.NewError("cannot convert %s to float: %s", arg.Type(), err)
	}
	return &object.Float{Value: val}
}

func toBoolObject(arg object.Object) object.Object {
	val, err := convert.ToBool(eval.ToNative(arg))
	if err != nil {
		return object.NewError("cannot convert %s to bool: %s", arg.Type(), err)
	}
	...
	eval.RegisterBuiltins(map[string]*object.Builtin{
		...
		"channel": {
			Fn: func(args ...object.Object) object.Object {
				...
				return &object.Channel{Ch: make(chan object.Object, buffer)}
			},
		},
		"send": {
			Fn: func(args ...object.Object) object.Object {
		...
		"os_env": {
			Fn: func(args ...object.Object) object.Object {
				if len(args) == 1 {
					...
					if err := security.EnvReadAllowed(key); err != nil {
						return &object.String{Value: fmt.Sprintf("ERROR: %s", err)}
					}
...
// the project/sandbox boundary. Symlinks are resolved on both the target path
// and the root before the jail check, so a symlink inside the allowed root
// that points outside of it cannot be used to escape the sandbox. If the
// target path does not exist yet (e.g. write_file creating a new file),
...
func SanitizePathLocal(userPath string) (string, error) {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			realPath = filepath.Join(realDir, filepath.Base(abs))
		} else {
			return "", err
		}
	}
	...
	if realRoot, rootErr := filepath.EvalSymlinks(root); rootErr == nil {
		root = filepath.Clean(realRoot)
	}

	// The path must be exactly the root or a child of it.
	if realPath != root && !strings.HasPrefix(realPath, root+string(os.PathSeparator)) {
