...
func (fw *FileWatcher) fireHandler(entry *WatchEntry, changedFiles []string) {
	...
	case *object.Function:
		if object.ExtendFunctionEnvFn != nil && object.EvalFn != nil {
			...
			// Fresh counters per fire: a watcher fires repeatedly for the
			// life of the process, so reusing entry.Env's RuntimeLimits
			// would accumulate steps until MaxSteps is exceeded forever.
			if entry.Env != nil && entry.Env.RuntimeLimits != nil {
				extEnv.RuntimeLimits = entry.Env.RuntimeLimits.CloneForConcurrentExecution()
			}
