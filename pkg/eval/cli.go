...
func StartCLI() {
	...
	var data string
	flag.StringVar(&data, "data", "", "Runtime data bound to the script's `data` global: inline JSON, @path/to/file (.json/.yaml/.yml/.env), or - to read JSON from stdin")
	flag.StringVar(&data, "d", "", "Shorthand for -data")
	flag.Parse()
	...
	go func() {
		...
		// Give registered servers/workers (see object.RegisterShutdownHook)
		// a bounded window to stop cleanly - e.g. an HTTP server draining
		// in-flight requests - rather than killing the process mid-request.
