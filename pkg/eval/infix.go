...
func valueMatchesTypeRef(val object.Object, ref *ast.TypeRef) bool {
	...
	if immutable, ok := val.(*object.ImmutableValue); ok {
		val = immutable.Inner
	}
	if len(ref.Union) > 0 {
		for _, alternative := range ref.Union {
			if valueMatchesTypeRef(val, alternative) {
				return true
			}
		}
		return false
	}
	isNull := val == nil || val == object.NULL || val.Type() == object.NULL_OBJ
	if ref.Nullable && isNull {
		return true
	}
	if ref.ArrayDepth > 0 {
		...
		elementRef := *ref
		elementRef.ArrayDepth--
		for _, element := range array.Elements {
			if !valueMatchesTypeRef(element, &elementRef) {
				return false
			}
		}
		return true
	}
	t := strings.ToLower(strings.TrimSpace(ref.Name))
	switch t {
	case "any", "object":
		return true
	case "int", "integer":
	...
	case "array", "list":
		...
		if len(ref.Arguments) > 0 {
			for _, element := range array.Elements {
				if !valueMatchesTypeRef(element, ref.Arguments[0]) {
					return false
				}
			}
		}
	...
	case "hash", "map":
		...
		for _, pair := range hash.Pairs {
			if len(ref.Arguments) > 0 && !valueMatchesTypeRef(pair.Key, ref.Arguments[0]) {
				return false
			}
			if len(ref.Arguments) > 1 && !valueMatchesTypeRef(pair.Value, ref.Arguments[1]) {
				return false
			}
		}
		return true
	...
	default:
		if instance, ok := val.(*object.ClassInstance); ok {
			return strings.EqualFold(instance.Class.Name, ref.Name)
		}
		if adt, ok := val.(*object.ADTValue); ok {
			return strings.EqualFold(adt.TypeName, ref.Name)
		}
		return true
	}
}
