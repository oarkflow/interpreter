...
func (p *Parser) parseStatement() ast.Statement {
	...
	case token.FOR:
		if p.peekTokenIs(token.AWAIT) {
			return p.parseForAwaitStatement()
		}
	...
	case token.IDENT:
		...
		if p.curToken.Literal == "class" || p.curToken.Literal == "abstract" {
			return p.parseClassStatement()
		}
...
func (p *Parser) parseLetStatement() ast.Statement {
	...
	if stmt.Name != nil && len(stmt.Names) == 0 && p.peekTokenIs(token.COLON) {
		...
		stmt.Type = ast.ParseTypeRef(typeName)
	}
...
func (p *Parser) parseConstStatement() ast.Statement {
	...
	stmt := &ast.LetStatement{IsConst: true}
...
func (p *Parser) parseImportStatement() *ast.ImportStatement {
	...
	if p.peekTokenIs(token.LBRACE) {
		...
		for {
			...
			imported := p.internIdentifier(p.curToken.Literal)
			local := imported
			if p.peekTokenIs(token.IDENT) && strings.EqualFold(p.peekToken.Literal, "as") {
				p.nextToken()
				if !p.expectPeek(token.IDENT) {
					return nil
				}
				local = p.internIdentifier(p.curToken.Literal)
			}
			stmt.Names = append(stmt.Names, imported)
...
func (p *Parser) parseExportStatement() *ast.ExportStatement {
	...
	if p.peekTokenIs(token.ABSTRACT) {
		p.nextToken()
		stmt.Declaration = p.parseClassStatement()
		return stmt
	}
	...
	if p.peekTokenIs(token.IDENT) {
		...
		case "class":
			stmt.Declaration = p.parseClassStatement()
		case "interface":
			stmt.Declaration = p.parseInterfaceStatement()
		case "type":
			stmt.Declaration = p.parseTypeDeclarationStatement()
		default:
			p.errors = append(p.errors, fmt.Sprintf("Line %d: unsupported export declaration %q", p.curToken.Line, p.curToken.Literal))
			return nil
		}
		return stmt
	}

	msg := fmt.Sprintf("Line %d: export must be followed by a declaration", p.curToken.Line)
	p.errors = append(p.errors, msg)
	return nil
}
...
func (p *Parser) parseExpressionStatement() *ast.ExpressionStatement {
	...
	macroArity := 0
	if p.curTokenIs(token.IDENT) {
		macroName = p.curToken.Literal
		macroArity = p.macros[macroName]
	}
	if macroArity > 0 && !p.peekTokenIs(token.LPAREN) {
		call := &ast.MacroCallExpression{Name: p.internIdentifier(macroName)}
		for i := 0; i < macroArity; i++ {
			p.nextToken()
			// Bare DSL-style macro arguments are deliberately atomic. Complex
			// expressions remain available through the unambiguous name(...)
			// form; this prevents the next line from being consumed as postfix
			// syntax on the final bare argument.
			arg := p.parseExpression(POSTFIX)
			if arg == nil {
				return stmt
			}
	...
	if macroArity > 0 {
		if call, ok := stmt.Expression.(*ast.CallExpression); ok {
			macroCall := &ast.MacroCallExpression{Name: p.internIdentifier(macroName), Arguments: call.Arguments}
			if p.peekTokenIs(token.LBRACE) {
				p.nextToken()
				macroCall.Block = p.parseBlockStatement()
			}
			stmt.Expression = macroCall
		}
	}
...
func (p *Parser) parseExpression(precedence int) ast.Expression {
	...
	case token.SUPER:
		leftExp = p.parseSuperExpression()
	default:
	...
	for !p.peekTokenIs(token.SEMICOLON) && precedence < p.peekPrecedence() {
		...
		case token.LARROW:
			p.nextToken()
			leftExp = p.parseChannelExpression(leftExp)
		default:
			return leftExp
		}
	}
...
func (p *Parser) parseChannelExpression(channel ast.Expression) ast.Expression {
	if p.peekTokenIs(token.SEMICOLON) || p.peekTokenIs(token.RPAREN) ||
		p.peekTokenIs(token.RBRACE) || p.peekTokenIs(token.COMMA) || p.peekTokenIs(token.EOF) {
		return &ast.ChannelReceiveExpression{Channel: channel}
	}
	p.nextToken()
	return &ast.ChannelSendExpression{Channel: channel, Value: p.parseExpression(LOWEST)}
}
...
func (p *Parser) parseArrowFunction() ast.Expression {
	...
	lit.ParamTypeRefs = parseTypeRefs(lit.ParamTypes)
	if !p.expectPeek(token.ARROW) {
		return nil
	}
	p.nextToken() // advance past =>
...
func (p *Parser) parseFunctionLiteral() ast.Expression {
	...
	if p.peekTokenIs(token.COLON) {
		...
		lit.ReturnTypeRef = ast.ParseTypeRef(lit.ReturnType)
	}
...
func (p *Parser) parseTypeName() string {
	...
	appendToken := func(tok token.Token) {
		if tok.Type == token.BITOR {
			out.WriteString(" | ")
			return
		}
	...
	updateDepth := func(tok token.Token) {
		switch tok.Type {
		case token.LT:
			angleDepth++
		case token.GT:
	...
	appendToken(p.curToken)
	updateDepth(p.curToken)
	for isTypeContinuation(p.peekToken.Type, angleDepth, parenDepth, bracketDepth) {
		p.nextToken()
		appendToken(p.curToken)
		updateDepth(p.curToken)
	}
...
func isTypeToken(t token.TokenType) bool {
	switch t {
	case token.IDENT, token.NULL, token.FUNCTION, token.LPAREN:
		return true
	default:
		return false
	}
...
func isTypeContinuation(t token.TokenType, angleDepth, parenDepth, bracketDepth int) bool {
	switch t {
	case token.IDENT, token.NULL, token.FUNCTION, token.LT, token.GT, token.RSHIFT,
		token.BITOR, token.QUESTION, token.LBRACKET, token.DOT, token.LPAREN:
		return true
	case token.COMMA:
		return angleDepth > 0 || parenDepth > 0
	case token.RBRACKET:
...
func parseTypeRefs(names []string) []*ast.TypeRef {
	refs := make([]*ast.TypeRef, len(names))
	for i, name := range names {
		refs[i] = ast.ParseTypeRef(name)
	}
	return refs
}
...
func (p *Parser) parseClassStatement() ast.Statement {
	...
	isAbstract := false
	if p.curToken.Literal == "abstract" || (p.curTokenIs(token.IDENT) && p.curToken.Literal == "abstract") {
		isAbstract = true
	...
	if p.peekTokenIs(token.EXTENDS) {
		...
		stmt.Parent = p.internIdentifier(p.curToken.Literal)
	}

	if p.peekTokenIs(token.IDENT) && p.peekToken.Literal == "implements" {
		p.nextToken() // consume "implements"
		for {
			if !p.expectPeek(token.IDENT) {
				return nil
			}
			stmt.Implements = append(stmt.Implements, p.internIdentifier(p.curToken.Literal))
			if !p.peekTokenIs(token.COMMA) {
				break
			}
			p.nextToken() // consume comma
	...
	for !p.curTokenIs(token.RBRACE) && !p.curTokenIs(token.EOF) {
		...
		memberIsAbstract := false
		memberIsPrivate := false
		for {
			...
			if p.curTokenIs(token.ABSTRACT) || (p.curTokenIs(token.IDENT) && p.curToken.Literal == "abstract") {
				memberIsAbstract = true
				p.nextToken()
				continue
			}
			if p.curTokenIs(token.IDENT) && p.curToken.Literal == "private" {
		...
		if p.curTokenIs(token.INIT) || (p.curTokenIs(token.IDENT) && p.curToken.Literal == "init") {
			m := p.parseClassMethodBody()
			m.IsConstructor = true
			m.IsPrivate = memberIsPrivate
			stmt.Methods = append(stmt.Methods, m)
		} else if p.curTokenIs(token.IDENT) {
			if p.peekTokenIs(token.LPAREN) {
				m := p.parseClassMethodBody()
				m.IsStatic = memberIsStatic
				m.IsAbstract = memberIsAbstract
				m.IsPrivate = memberIsPrivate
				if memberIsAbstract {
					m.Body = nil
				}
				stmt.Methods = append(stmt.Methods, m)
			} else if p.peekTokenIs(token.ASSIGN) || p.peekTokenIs(token.SEMICOLON) {
				f := &ast.ClassField{
					Name:      p.internIdentifier(p.curToken.Literal),
					IsStatic:  memberIsStatic,
					IsPrivate: memberIsPrivate,
				}
				if p.peekTokenIs(token.ASSIGN) {
					p.nextToken() // consume name -> now at ASSIGN
					p.nextToken() // consume ASSIGN -> now at value start
				...
				p.errors = append(p.errors, fmt.Sprintf("Line %d: expected '(' or '=' or ';' after class member name, got %s", p.curToken.Line, token.Debug(p.peekToken)))
				return nil
			}
		} else {
...
func (p *Parser) parseClassMethodBody() *ast.ClassMethod {
	...
	m.ParamTypeRefs = parseTypeRefs(paramTypes)
	m.Defaults = defaults
	m.HasRest = hasRest
	if p.peekTokenIs(token.COLON) {
		p.nextToken()
		m.ReturnType = p.parseTypeName()
		m.ReturnTypeRef = ast.ParseTypeRef(m.ReturnType)
	}
	if p.peekTokenIs(token.LBRACE) {
		p.nextToken()
...
func (p *Parser) parseMacroDefinition() ast.Statement {
	stmt := &ast.MacroDefinition{}
	if p.curTokenIs(token.MACRO) {
		if !p.expectPeek(token.IDENT) {
			return nil
		}
	} else if p.curTokenIs(token.IDENT) && p.curToken.Literal == "macro" {
		if !p.expectPeek(token.IDENT) {
			return nil
		}
	}
	...
	if !p.peekTokenIs(token.RPAREN) {
		p.nextToken()
		stmt.Parameters = append(stmt.Parameters, p.internIdentifier(p.curToken.Literal))
		for p.peekTokenIs(token.COMMA) {
			p.nextToken()
			p.nextToken()
			stmt.Parameters = append(stmt.Parameters, p.internIdentifier(p.curToken.Literal))
	...
	stmt.Body = p.parseBlockStatement()
	p.macros[stmt.Name.Name] = len(stmt.Parameters)
	return stmt
}

func (p *Parser) parseForAwaitStatement() *ast.ForAwaitStatement {
	stmt := &ast.ForAwaitStatement{}
	...
	if p.curTokenIs(token.IDENT) {
		if p.peekTokenIs(token.COMMA) {
			stmt.KeyName = p.internIdentifier(p.curToken.Literal)
			p.nextToken() // comma
			p.nextToken()
			stmt.ValueName = p.internIdentifier(p.curToken.Literal)
		} else {
			stmt.ValueName = p.internIdentifier(p.curToken.Literal)
		}
	}
...
func (p *Parser) parseSelectStatement() *ast.SelectStatement {
	...
	for !p.curTokenIs(token.RBRACE) && !p.curTokenIs(token.EOF) {
		if p.curTokenIs(token.DEFAULT) {
			if !p.expectPeek(token.COLON) || !p.expectPeek(token.LBRACE) {
				return nil
			}
		...
		if !p.curTokenIs(token.CASE) {
			p.errors = append(p.errors, fmt.Sprintf("Line %d: expected 'case' in select, got %s", p.curToken.Line, token.Debug(p.curToken)))
			p.nextToken()
			continue
		}
		p.nextToken() // consume case
		sc := &ast.SelectCase{}
		sc.Channel = p.parseExpression(ASSIGN_PREC)
		if p.peekTokenIs(token.LARROW) {
			p.nextToken() // <-
			p.nextToken()
			sc.Value = p.internIdentifier(p.curToken.Literal)
			sc.IsRecv = true
		}
		if !sc.IsRecv {
			p.errors = append(p.errors, fmt.Sprintf("Line %d: select case requires '<- name'", p.curToken.Line))
			return nil
		}
		if !p.expectPeek(token.COLON) {
			return nil
...
func (p *Parser) parseSpawnExpression() ast.Expression {
	...
	funcExpr := p.parseExpression(PREFIX)
	if funcExpr == nil {
		return nil
	}
	// Calls bind more tightly than the spawn prefix. Preserve their function
	// and arguments separately so `spawn work(42)` schedules work instead of
	// eagerly evaluating work(42) and attempting to call its result.
	if call, ok := funcExpr.(*ast.CallExpression); ok {
		return &ast.SpawnExpression{Function: call.Function, Args: call.Arguments}
	}
	se := &ast.SpawnExpression{Function: funcExpr}
	if p.peekTokenIs(token.LPAREN) {
		p.nextToken()
		se.Args = p.parseExpressionList(token.RPAREN)
	}
	return se
}
...
func (p *Parser) parseInterfaceStatement() ast.Statement {
	...
	for !p.curTokenIs(token.RBRACE) && !p.curTokenIs(token.EOF) {
		...
		stmt.Methods = append(stmt.Methods, &ast.InterfaceMethod{Name: methodName, Parameters: params, ParamTypes: paramTypes, ParamTypeRefs: parseTypeRefs(paramTypes), ReturnType: retType, ReturnTypeRef: ast.ParseTypeRef(retType)})
		p.nextToken()
	}
	return stmt
}
...
func (p *Parser) expectPeekMemberName() bool {
	if p.peekTokenIs(token.IDENT) || (p.peekToken.Type >= token.LET && p.peekToken.Type <= token.SUPER) {
		p.nextToken()
		return true
	}
	p.peekError(token.IDENT)
...
func (p *Parser) parseSuperExpression() ast.Expression {
	superIdent := &ast.Identifier{Name: "super"}
	if p.peekTokenIs(token.LPAREN) {
		p.nextToken()
	...
	if p.peekTokenIs(token.DOT) {
		...
		return &ast.DotExpression{
			Left:  superIdent,
			Right: p.internIdentifier(p.curToken.Literal),
		}
	}
	return superIdent
}
