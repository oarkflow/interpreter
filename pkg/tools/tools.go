...
// deriveScryptKey derives a 32-byte AES-256 key from a passphrase and salt
// using scrypt, making offline brute-force/dictionary attacks against weak
// passphrases significantly more expensive than a bare SHA-256 hash.
...
func cryptFile(opName, src, dst, passphrase string, apply bool, h Hooks) Operation {
	...
	if opName == "encrypt" {
		...
		out, err = gcm.Open(nil, nonce, rest[gcm.NonceSize():], nil)
		if err != nil {
			op.Status, op.Error = "failed", err.Error()
			return op
		}
