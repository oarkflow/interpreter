...
func EvalForPlayground(script string, opts PlaygroundOptions) PlaygroundResult {
	...
	if policy == nil {
		policy = &SecurityPolicy{ProtectHost: true}
	}

	_, err := withSecurityPolicyOverride(policy, func() (Object, error) {
		env := NewGlobalEnvironment(opts.Args)
		if opts.ModuleDir != "" {
		...
		if len(p.Errors()) != 0 {
			res.Error = formatParserErrors(p.Errors())
			res.ErrorKind = "parser"
			res.Diagnostics = append(res.Diagnostics, p.Errors()...)
		...
		if evaluated != nil {
			if isError(evaluated) {
				res.Error = objectErrorString(evaluated)
				res.ErrorKind = "runtime"
				if errObj, ok := evaluated.(*Error); ok && len(errObj.Stack) > 0 {
					res.Diagnostics = append(res.Diagnostics, formatCallStack(errObj.Stack))
				}
			} else {
