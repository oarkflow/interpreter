...
func RegisterStdBuiltinModule(name string, builtinNames ...string) error {
	return RegisterStdBuiltinModuleWithPrefix(name, "", builtinNames...)
}

// RegisterStdBuiltinModuleWithPrefix registers a std module (reachable only
// via `import "<name>" ...;`) backed by already-registered builtins looked
// up by their full name. When prefix is non-empty, exported keys have the
// prefix stripped (e.g. prefix "rules_" turns "rules_service" into the
// module member "service", so `rules.service()` works after
// `import "rules" as rules;`); a builtin name that doesn't start with the
// prefix keeps its full name in the export map.
...
func LookupStdModule(name string) (map[string]Object, bool) {
	...
	if builtinOK {
		// A stripped key must be unique within this module: if two builtin
		// names would strip to the same key (e.g. "db_query" -> "query"
		// colliding with the already-unprefixed "query"), keep the full
		// name for every entry involved in that collision instead of
		// letting one silently shadow the other.
		keyCounts := make(map[string]int, len(builtinNames))
		for _, builtinName := range builtinNames {
			key := builtinName
			if prefix != "" && strings.HasPrefix(builtinName, prefix) {
				key = strings.TrimPrefix(builtinName, prefix)
			}
			keyCounts[key]++
		}
		...
		for _, builtinName := range builtinNames {
			key := builtinName
			if prefix != "" && strings.HasPrefix(builtinName, prefix) {
				if stripped := strings.TrimPrefix(builtinName, prefix); keyCounts[stripped] == 1 {
					key = stripped
				}
			}
			if fn, ok := eval.BuiltinByName(builtinName); ok {
				...
				out[key] = unavailableBuiltin(name, builtinName)
			}
		}
		if len(out) == 0 {
			return nil, false
...
func unavailableBuiltin(moduleName, builtinName string) *object.Builtin {
	return &object.Builtin{Fn: func(args ...object.Object) object.Object {
		return object.NewError("optional builtin %q from module %q is not linked into this interpreter; use cmd/interpreter (built with the full plugin set), import the optional Go package in your embedding host, or build a custom preset", builtinName, moduleName)
	}}
}
...
func init() {
	...
	_ = RegisterStdBuiltinModuleWithPrefix("database", "db_", "db_connect", "db_query", "db_exec", "db_begin", "db_commit", "db_rollback", "db_tables", "db_close", "query", "lazy_query")
	_ = RegisterStdBuiltinModuleWithPrefix("images", "image_", "image_load", "image_resize", "image_crop", "image_rotate", "image_convert", "image_save", "image_info", "image_render", "image_resize_file", "image_convert_file")
	...
	_ = RegisterStdBuiltinModuleWithPrefix("naturaldate", "naturaldate_", "naturaldate_parse", "naturaldate_parse_all")
	_ = RegisterStdBuiltinModuleWithPrefix("wuid", "wuid_", "wuid_new", "wuid_new_uuid", "wuid_parse")
	_ = RegisterStdBuiltinModuleWithPrefix("money", "money_", "money_new", "money_add", "money_sub", "money_mul", "money_percent", "money_format")
	_ = RegisterStdBuiltinModuleWithPrefix("phone", "phone_", "phone_parse", "phone_valid", "phone_country", "phone_networks", "phone_parse_bulk")
	_ = RegisterStdBuiltinModuleWithPrefix("ip", "ip_", "ip_is_private", "ip_client_from_header", "ip_geo_init", "ip_country", "ip_lookup", "ip_lookup_bulk")
	_ = RegisterStdBuiltinModuleWithPrefix("shamir", "shamir_", "shamir_split", "shamir_combine")
	_ = RegisterStdBuiltinModule("metadata", "infer_csv_types", "infer_json_types", "infer_value_type")
	_ = RegisterStdBuiltinModuleWithPrefix("yaml", "yaml_", "config_load", "config_parse", "yaml_encode", "yaml_decode")
	_ = RegisterStdBuiltinModuleWithPrefix("config/yaml", "yaml_", "config_load", "config_parse", "yaml_encode", "yaml_decode")

	_ = RegisterStdBuiltinModuleWithPrefix("pdf", "pdf_",
		...
		"pdf_to_docx", "pdf_markdown_to_docx", "pdf_from_docx", "pdf_docx_to_markdown", "pdf_docx_to_text",
		"pdf_docx_to_html", "pdf_html_to_docx",
		"pdf_to_json", "pdf_to_text", "pdf_search", "pdf_extract_images", "pdf_images_to_pdf", "pdf_list_form_fields",
		"pdf_fill_form", "pdf_quick",
	)
	_ = RegisterStdBuiltinModuleWithPrefix("rules", "rules_", "rules_service", "rules_publish", "rules_evaluate", "rules_activate", "rules_rollback")
	_ = RegisterStdBuiltinModuleWithPrefix("secretr", "secretr_", "secretr_get", "secretr_set", "secretr_delete", "secretr_list", "secretr_scan")
	_ = RegisterStdBuiltinModule("server", "server", "route", "middleware", "native_middleware", "static", "template_dir", "web_app", "route_group", "group", "listen", "listen_async", "shutdown")
	_ = RegisterStdBuiltinModuleWithPrefix("tcpguard", "tcpguard_", "tcpguard_load", "tcpguard_new", "tcpguard_evaluate", "guard_middleware")
	_ = RegisterStdBuiltinModuleWithPrefix("emailvalidator", "email_", "email_validate", "email_validate_syntax", "email_is_disposable", "email_is_role_account", "email_is_free_provider", "email_validate_bulk")
}
