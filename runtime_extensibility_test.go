...
func TestImportPolicyAndModuleLockVerification(t *testing.T) {
	...
	// Restore the original content: the remaining assertions test the
	// import-package policy denial path independently of checksum
	// verification (which now also runs during ResolveManifestImport and
	// would otherwise short-circuit with a checksum-mismatch error first).
	if err := os.WriteFile(filepath.Join(dep, "math.spl"), []byte(`export let answer = 42;`), 0o644); err != nil {
		t.Fatal(err)
	}
