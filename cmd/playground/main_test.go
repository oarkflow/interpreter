...
func TestPlaygroundLinksToolsBuiltins(t *testing.T) {
	for _, name := range []string{"bulk_rename", "archive_compress", "image_convert_batch", "media_convert", "ffmpeg_status"} {
		if !eval.HasBuiltin(name) {
			t.Fatalf("expected tools builtin %q to be linked into cmd/playground", name)
		}
	}
}
...
func TestReactiveHTMLExampleUsesRawHTMLAndRuns(t *testing.T) {
	src := builtinCodeExamples()["reactive-html"]
	...
	res := interpreter.EvalForPlayground(src, interpreter.PlaygroundOptions{
		...
		Security:  playgroundSecurityPolicyForProfile(filepath.Join("..", ".."), "untrusted", false, nil),
