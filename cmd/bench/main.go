...
func main() {
	...
	if err := run("SPL vs expr-lang/expr", "-C", "benchmarks/exprcompare", "test", ".", "-run", "^$", "-bench", "^BenchmarkExprComparison", "-benchmem", "-count", "5"); err != nil {
		fmt.Fprintf(os.Stderr, "Expr comparison benchmark run failed: %v\n", err)
		os.Exit(1)
	}
}
